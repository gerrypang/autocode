...
public class DataBaseSetUpHandler extends BaseSetUpHandler {
	...
	public DataSourceModel loadDataSourceConfig() {
		...
		if (DatabaseType.MYSQL.equalsIgnoreCase(dataSource.getDataType())) {
			...
			dataSource.setServerFullUrl(propertyMap.get(AutoCodeProperties.JDBC_URL) + DatabaseURL.MYSQL_URL);
		} else {
			dataSource.setServerUrl(propertyMap.get(AutoCodeProperties.JDBC_URL));
			dataSource.setServerFullUrl(propertyMap.get(AutoCodeProperties.JDBC_URL));
		}
